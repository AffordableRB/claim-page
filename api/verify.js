...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!response.ok) {
      return res.status(500).json({ error: `Failed to fetch search page: HTTP ${response.status}` });
    }

    const html = await response.text();

    // Basic check for Roblox anti-bot or block page
    const lowerHtml = html.toLowerCase();
    if (lowerHtml.includes('captcha') || lowerHtml.includes('access denied') || lowerHtml.includes('blocked')) {
      return res.status(403).json({ error: 'Roblox blocked the request or captcha required' });
    }

    // Parse first user using regex
